...
sep  = os.path.sep
footages = f"{os.getcwd()}{sep}data{sep}footages{sep}"
data_dir = f"{os.getcwd()}{sep}data{sep}users{sep}ID1{sep}"
data_dir_img = f"{os.getcwd()}{sep}data{sep}users{sep}ID1{sep}img{sep}"
out_file = f"{os.getcwd()}{sep}data{sep}users{sep}ID1{sep}result{sep}out.mp4"
...
# картинки
images = [f"{data_dir_img}{i}" for i in os.listdir(data_dir_img) if i.endswith(".jpg")]
...
cmd += f"-i {data_dir}music{sep}music.mp3   "
cmd += f" -i {footages}2.mp4 -loop 1   "
